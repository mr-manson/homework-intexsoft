...
const getSMS = (arr, it = 1, itEndSize = 1) => {
  var iteration = it;
  var iterationEndSize = itEndSize;
  ...
  while (smsText.length < 140 && count < arr.length) {
    if (
      (smsText + arr[count] + " " + iteration + "/").length +
        iterationEndSize <=
      140
    ) {
      smsText += arr[count] + " ";
  ...
  var iterationResult = getSMS(arr.slice(count), ++iteration, iterationEndSize);
  var result = [smsText].concat(iterationResult.result);
  if (result[0]) result[0] += totalIterationCount;
  return { result: result, needNewRecursion: iterationResult.needNewRecursion };
};
